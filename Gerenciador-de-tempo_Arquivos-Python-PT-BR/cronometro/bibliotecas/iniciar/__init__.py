...
def program(window, a, s, active, reset, res):
    # Define as váriaveis como sendo globais
    ...
    def relogio():
        ...
        cr_active = False # Variável para saber se a função relogio() está em execução
        r_cont += 1 # Variável para saber a quantidade de vezes que a função relógio foi chamada

        # Se a função relógio já tiver sido chamada anteriormente ele irá destruir o antigo Label, para construir um novo
        if r_cont >= 2:
            h.destroy()

        # Destrói as variáveis text, b_start, b_stop e b_reset, que foram definidas na função cron()
        text.destroy()
        b_start.destroy()
        b_stop.destroy()
        b_reset.destroy()
        ...
        window.iconbitmap('cronometro\\media\\icons\\icons\\main.ico') # Especifique o local exato do arquivo main.ico EX.: 'C:\\Users\\Usuario1\\project\\cronometro\\media\\icons\\icons\\main.ico'
        window.update_idletasks() # Atualiza a janela e todos os seus filhos
        while True:
            format_hora = datetime.now().strftime('%H:%M:%S') # Obtém a hora atual do sistema
            h = Label(window, text = format_hora, 
                      font = ('LCDMono2', (e + 60), 'bold'), 
                      bg = palet_color[0]['BG'], fg = palet_color[0]['FG']) # Cria um Label com a hora atual do sistema
            h.pack(expand = True, anchor = CENTER) # Posiciona o Label na janela
            window.update() # Atualiza a janela
            h.destroy() # Destrói o Label com a hora atual, para quando o while for executado novamente ele não crie outro por cima do que já existe

            if cr_active == True:
                break
        else:
            h.destroy() # Quando o loop for finalizado ele destruirá o Label com a hora atual

    def prefs():
        global cont_prefs;global p;global white;global black;global bt_ok

        cont_prefs += 1 # Variável para saber a quantidade de vezes que a função prefs() foi chamada

        p = Toplevel(window) # Cria uma janela superior "filha" da janela window
        p['bg'] = palet_color[0]['BG'] # Define a cor de fundo da janela toplevel
        p.title('Preferências') # Define o título da janela
        p.geometry('400x350+540+210') # Define a geometria
        p.resizable(0, 0) # Desativa o botão de maximizar
        p.grab_set() # Define a janela como principal, o usuário não poderá interagir com outras telas enquanto esta janela estiver aberta
        # Define o ícone da janela
        p.iconbitmap('cronometro\\media\\icons\\icons\\prefs.ico') # Especifique o local exato do arquivo prefs.ico EX.: 'C:\\Users\\Usuario1\\project\\cronometro\\media\\icons\\icons\\prefs.ico'

        def interface(value):
            # Define as variáveis p, white, black, bt_ok e p_msg como sendo globais
            global p;global white;global black;global bt_ok;global p_msg
            p.config(bg = palet_color[value]['BG']) # Define a cor de fundo da interface

            p_msg = Label(p, text = 'OBS. ESTA AREA AINDA NAO ESTA DISPONIVEL \nPOIS ESTA SENDO DESENVOLVIDA', 
                          font = ('LCDMono2', 13), bg = palet_color[0]['BG'], fg = palet_color[0]['FG']) # Cria um Label com uma mensagem de text
            p_msg.pack(side = TOP, anchor = NW, fill = X) # Posiciona o Label na janela

            white = Radiobutton(p, text = 'Tema claro', variable = var, value = 1, command = None, 
                                bg = palet_color[value]['BG'], fg = palet_color[value]['FG'], 
                                activebackground = palet_color[value]['BG'], 
                                activeforeground = palet_color[value]['AC_FG'], state = DISABLED) # Cria um Radiobutton
            white.pack(side = TOP, anchor = W, fill = X) # Posiciona o Radiobutton na janela
            ...
            black = Radiobutton(p, text = 'Tema escuro', variable = var, value = 2, command = None, bg = palet_color[value]['BG'], 
                                ...
                                activeforeground = palet_color[value]['AC_FG'], state = DISABLED) # Cria um Radiobutton
            black.pack(side = TOP, anchor = W, fill = X) # Posiciona o Radiobutton na janela

            bt_ok = Button(p, text = 'Ok', bg = palet_color[value]['BG'], fg = palet_color[value]['FG'], 
                           width = 10, bd = .5, command = p.destroy, 
                           activebackground = palet_color[value]['BG'], 
                           activeforeground = palet_color[value]['AC_FG']) # Cria o botão Ok            
            bt_ok.pack(side = BOTTOM, anchor = SE) # Posiciona o Botão Ok na janela

        interface(0) # Chama a função interface(0) passando como paramêtro o valor 0
        black.select() # Seleciona o Radiobutton black
        p.mainloop() # Roda a janela em loop
    ...
    def criar_alarme():
        ...
        alarm_managment = Toplevel(window) # Cria uma janela superior "filha" da janela window
        alarm_managment.title('Gerenciador de alarmes') # Define o título da janela
        alarm_managment.geometry('400x350+540+210') # Define a geometria da janela
        alarm_managment['bg'] = palet_color[0]['BG'] # Define a cor de fundo da janela
        alarm_managment.resizable(0, 0) # Desativa o botão de maximizar
        alarm_managment.grab_set() # Define a janela como principal, o usuário não poderá interagir com outras telas enquanto esta janela estiver aberta
        alarm_managment.iconbitmap('cronometro\\media\\icons\\icons\\alarm.ico') # Especifique o local exato do arquivo alarm.ico EX.: 'C:\\Users\\Usuario1\\project\\cronometro\\media\\icons\\icons\\alarm.ico'
        ...
        def criar():
            ...
            h = m = s1 = s2 = ''
            cont += 1
            # Configura os botões, radiobuttons, etc. como desativados
        ...
        rigid_lapis = PhotoImage(file = 'cronometro\\media\\icons\\lapis.png') # Especifique o local exato do arquivo lapis.png EX.: 'C:\\Users\\Usuario1\\project\\cronometro\\media\\icons\\icons\\lapis.png'
        rigid_lixeira = PhotoImage(file = 'cronometro\\media\\icons\\lixeira.png') # Especifique o local exato do arquivo lixeira.png EX.: 'C:\\Users\\Usuario1\\project\\cronometro\\media\\icons\\icons\\lixeira.png'
        rigid_warning = PhotoImage(file = 'cronometro\\media\\icons\\warning.png') # Especifique o local exato do arquivo lixeira.png EX.: 'C:\\Users\\Usuario1\\project\\cronometro\\media\\icons\\icons\\lixeira.png'
        ...
        def okay():
            ...
            i = PhotoImage(file = 'cronometro\\media\\icons\\info.png') # Especifique o local exato do arquivo info.png EX.: 'C:\\Users\\Usuario1\\project\\cronometro\\media\\icons\\icons\\info.png'
    ...
    def cron():
        ...
        window.iconbitmap('cronometro\\media\\icons\\icons\\main_window.ico') # Especifique o local exato do arquivo main_window.ico EX.: 'C:\\Users\\Usuario1\\project\\cronometro\\media\\icons\\icons\\main_window.ico'
